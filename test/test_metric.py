...
def test_map_ndcg_y_rating():
    ...
    idcg = np.array([ s/np.log2(i+2) for i,s in enumerate( scores[np.argsort(scores)[::-1]][:K] ) ]).sum()
    expected_ndcg = (scores[0]/np.log2(0+2) + scores[1]/np.log2(1+2) + scores[2]/np.log2(2+2)) / idcg
    expected_map = (1 + 1 + 1) / K
    np.testing.assert_almost_equal(metric["ndcg"], expected_ndcg)
    np.testing.assert_almost_equal(metric["ap"], expected_map)
