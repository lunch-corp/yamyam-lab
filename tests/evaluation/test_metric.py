...
def test_ndcg_same_as_sklearn_binary_rel():
    ...
    number of recommended items (top k value): starting from 1 to 5
...
def test_map_ndcg_identical_btw_unvec_and_vec():
    liked_items = np.array([100, 10, 50, 0, 11, 22, 33, 44, 55, 66])
    # case 1: hit at item_id 100, 10
    reco_items = np.array([2, 3, 4, 100, 10])
    metric_unvec = ranking_metrics_at_k(liked_items, reco_items)

    liked_items = liked_items.reshape(1, len(liked_items))
    reco_items = reco_items.reshape(1, len(reco_items))
    metric_vec = fully_vectorized_ranking_metrics_at_k(liked_items, reco_items)

    assert metric_unvec["ap"] == metric_vec["ap"].sum()
    assert metric_unvec["ndcg"] == metric_vec["ndcg"].sum()
    assert metric_unvec["recall"] == metric_vec["recall"].sum()
