...
def save_all_similar_items(
    ...
    all_item_ids = [
        model.idx_to_item[idx] for idx in target_indices 
    ...
    valid_candidate_ids = set([
        model.idx_to_item[idx] for idx in valid_candidate_indices 
        if idx in model.idx_to_item
    ])
    
    print(f"Target restaurants: {len(all_item_ids):,} (reviews > 1)")
    print(f"Candidate restaurants: {len(valid_candidate_ids):,} (reviews >= {min_interactions})")
    ...
    for i, target_id in enumerate(all_item_ids, 1):
        ...
        if len(selected_items) < top_k:
            if warning_count < 5:
                print(f"  Warning: {target_id} has only {len(selected_items)} similar items")
            warning_count += 1
        
        results[target_id] = [
            [item['item_id'], round(item['hybrid_score'], 2)]
    ...
    if total_pairs > 0:
        avg_similarity = np.mean([score for items in results.values() for _, score in items if items])
...
if __name__ == "__main__":
    ...
    demo_parser = subparsers.add_parser("demo", help="Find similar items for random restaurants")
    ...
    find_parser = subparsers.add_parser("find", help="Find similar restaurants for a specific restaurant ID")
    find_parser.add_argument("--target_id", type=int, required=True)
    find_parser.add_argument("--top_k", type=int, default=10)
    find_parser.add_argument("--method", type=str, choices=["cosine_matrix", "jaccard"], default="cosine_matrix")
    find_parser.add_argument("--min_interactions", type=int, default=10, help="Minimum reviews for candidate restaurants")

    save_all_parser = subparsers.add_parser("save_all", help="Save all similar items using hybrid method")
    save_all_parser.add_argument("--top_k", type=int, default=10)
    save_all_parser.add_argument("--method", type=str, default="cosine_matrix", choices=["cosine_matrix", "jaccard"])
    save_all_parser.add_argument("--min_interactions", type=int, default=10)
    save_all_parser.add_argument("--cf_weight", type=float, default=0.8, help="CF similarity weight")
    save_all_parser.add_argument("--content_weight", type=float, default=0.2, help="Content similarity weight")
