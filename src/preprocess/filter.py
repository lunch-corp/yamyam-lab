...
class Filter:
    ...
    def filter_martial_law_reviews(
        ...
        self.logger.info(f"Token time for tokenizing: {round(time.time() - start, 2)}")

        # tag if each review is talking about martial law or not by overlapping word
        review_filtered["is_martial_law_review"] = review_filtered["token_count"].map(
            lambda x: len(set(x.keys()) & set(abusive_words)) >= 1
        )
